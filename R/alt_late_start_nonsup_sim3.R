         ...
         as_tibble(do.call(rbind, map(
           mclapply(
             ...
             function(j) run_a_noninf_trial_alt(
               j, c(1, rep(1.1, n_sup), rep(1, 12 - n_sup)),
...
saveRDS(res, "out/nonsuperiority/alt_late_start_mu_three_sup0.rds")
  ...
  assign(paste0("res", a),
         as_tibble(do.call(rbind, map(
           mclapply(
             1:1000,
             function(j) run_a_noninf_trial_alt(
               j, c(1, rep(1.25, n_sup), rep(1, 12 - n_sup)),
               delta_sup = pars$delta_sup[a],
               delta_noninf = pars$delta_noninf[a],
               kappa_act_0 = pars$kappa_act[a], kappa_act_1 = pars$kappa_act[a],
               kappa_sup_0 = pars$kappa_sup_0[a], kappa_sup_1 = pars$kappa_sup_1[a],
...
res <- bind_rows(lapply(1:nrow(pars), function(x) get(paste0("res", x))), .id = "pars")
rm(list = paste0("res", 1:nrow(pars)))
saveRDS(res, "out/nonsuperiority/alt_late_start_mu_three_sup1.rds")
  ...
  assign(paste0("res", a),
         as_tibble(do.call(rbind, map(
           mclapply(
             1:1000,
             function(j) run_a_noninf_trial_alt(
               j, c(1, rep(1.5, n_sup), rep(1, 12 - n_sup)),
               delta_sup = pars$delta_sup[a],
               delta_noninf = pars$delta_noninf[a],
               kappa_act_0 = pars$kappa_act[a], kappa_act_1 = pars$kappa_act[a],
               kappa_sup_0 = pars$kappa_sup_0[a], kappa_sup_1 = pars$kappa_sup_1[a],
...
res <- bind_rows(lapply(1:nrow(pars), function(x) get(paste0("res", x))), .id = "pars")
rm(list = paste0("res", 1:nrow(pars)))
saveRDS(res, "out/nonsuperiority/alt_late_start_mu_three_sup2.rds")
